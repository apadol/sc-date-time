        ...
        '$scope', 'scDateTimeI18n', function(scope, scDateTimeI18n) {
          ...
          scope.display = {
            fullTitle: function() {
              ...
              if (scope._displayMode === 'full' && !scope._verticalMode) {
                return moment(scope.date).format("dddd D MMMM YYYY, " + _timeString);
              } else if (scope._displayMode === 'time') {
                return (moment(scope.date)).format(_timeString);
              } else if (scope._displayMode === 'date') {
                return (moment(scope.date)).format('ddd D MMM YYYY');
              } else {
            ...
            title: function() {
              if (scope._mode === 'date') {
                return moment(scope.date).format((scope._displayMode === 'date' ? 'dddd' : "dddd " + (scope._hours24 ? 'HH:mm' : 'h:mm a')));
              } else {
          ...
          scope.clock = {
            ...
            _incHours: function(inc) {
              if (scope._hours24) {
                scope.clock._hours = Math.max(0, Math.min(23, this._hours + inc));
                scope.clock.hours = scope.clock.fillzeros(scope.clock._hours);
              } else {
                scope.clock._hours = Math.max(1, Math.min(12, this._hours + inc));
                scope.clock.hours = scope.clock.fillzeros(scope.clock._hours);
              }
              if (isNaN(scope.clock_hours)) {
                return scope.clock_hours = 0;
              }
            },
          ...
          scope.$watch('clock.hours', function(val) {
            if (scope._hours24) {
              ...
              scope.clock._hours = Math.max(1, Math.min(12, parseInt(val)));
            }
            if (isNaN(scope.clock_hours)) {
              scope.clock_hours = 0;
            }
            if (scope.clock.fillzeros(scope.clock._hours) !== scope.clock.hours) {
              return scope.clock.hours = scope.clock.fillzeros(scope.clock._hours);
            }
          });
